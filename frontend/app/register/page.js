...
export default function RegisterPage(){
  ...
  return (
    <div className="max-w-md mx-auto bg-white rounded-lg shadow-lg border border-[#B9D4AA] p-8 mt-8">
      <h1 className="text-3xl font-bold mb-6 text-[#5A827E] text-center">Create Account</h1>
      {error && <div className="mb-4 p-3 bg-red-50 border border-red-200 text-red-600 text-sm rounded">{error}</div>}
      <form onSubmit={onSubmit} className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-[#5A827E] mb-2">Full Name</label>
          <input 
        ...
        <button 
          disabled={loading} 
          className="w-full bg-[#84AE92] text-white py-3 rounded-lg hover:bg-[#5A827E] transition-colors font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {loading ? 'Creating...' : 'Create Account'}
        </button>
      </form>
      <p className="text-sm text-[#5A827E] mt-6 text-center">
        Already have an account? <a className="text-[#84AE92] hover:text-[#5A827E] font-medium" href="/login">Sign In</a>
      </p>
    </div>
  );
}
